...
export async function PUT(req: Request, context: Ctx) {
    try {
        ...
        const updated = await httpBackend<SupplierDTO>(
            SERVER_ENDPOINTS.suppliers,
            `/edit-supplier?supplierId=${id}`,
            { method: "PUT", body, headers: { "Content-Type": "application/json" } }
        );
        return NextResponse.json(updated, { status: 200 });
    } catch (e: any) {
        console.error("PUT /api/suppliers error:", e?.message, e?.status);
...
export async function DELETE(_req: Request, context: Ctx) {
    try {
        ...
        await httpBackend<void>(
            SERVER_ENDPOINTS.suppliers,
            `/delete-supplier?supplierId=${id}`,
            { method: "DELETE" }
        );
        return NextResponse.json({ ok: true }, { status: 200 });
    } catch (e: any) {
        console.error("DELETE /api/suppliers error:", e?.message, e?.status);
        return handleApiError(e);
    }
}