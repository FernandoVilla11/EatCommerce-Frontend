...
export default function CreatePurchaseModal({
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (items.some(i => !i.itemName.trim())) {
            setError('Todos los insumos deben tener nombre');
            return;
        }
        setLoading(true);
        setError(null);
        try {
            await PurchasesService.registerPurchase({
                supplierId,
                ...
                <div className="flex justify-between items-center mb-6">
                    <div>
                        ...
                        <p className="text-sm text-gray-400 mt-1">
                            Proveedor: <span className="text-emerald-400">{supplierName}</span>
                        </p>
                    </div>
                    <button onClick={handleClose} className="text-gray-400 hover:text-white transition text-xl">✕</button>
                </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input
                                ...
                                placeholder="Ej: Compra semanal de carnes"
                                required
                            />
                        </div>
                        <div>
                    ...
                    <div>
                        ...
                        {items.length === 0 ? (
                            <div className="text-center py-6 border border-dashed border-white/10 rounded-lg text-sm text-gray-500">
                                No hay insumos agregados. Haz clic en "+ Agregar insumo".
                            </div>
                        ) : (
                            <div className="space-y-3">
                                ...
                                {items.map((item, index) => (
                                    <div
                                        key={index}
                                        className="grid grid-cols-12 gap-2 items-center bg-white/5 p-3 rounded-lg border border-white/10"
                                    >
                    ...
                    <div className="flex justify-end gap-3 pt-2">
                        ...
                        <button
                            ...
                            className="px-5 py-2 bg-emerald-600 hover:bg-emerald-500 rounded-lg transition text-sm font-medium disabled:opacity-50"
                        >
